...
| 2026-03-07 | v2.1 — ENG-VALUATION: `offer_id` Datenpfad fuer MOD-12 (DRAFT_INTAKE). Edge Function `sot-valuation-engine` laedt `acq_offers`-Daten + `extracted_data` und baut Snapshot via `buildSnapshotFromOffer`. Client-seitig: `buildSnapshotFromAcqOffer` in `src/engines/akquiseCalc/engine.ts`. Bewertung im Tab "Bewertung" jetzt ohne vorherige Kalkulation startbar (nur Adresse + Preis/Miete noetig). |
