/**
 * AgentSyncCard — Zone 2 variant for personal phone assistants.
 * Mirrors Zone 1's AgentSyncCard but uses user-scoped assistant_id.
 */
...
export function AgentSyncCard({ config, onRefresh }: Props) {
  ...
  const handleSync = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('sot-phone-agent-sync', {
        body: { action: 'sync', assistant_id: config.id },
      });
      ...
      <CardContent className="space-y-4">
        ...
        {!promptText && (
          <p className="text-xs text-muted-foreground">
            Noch kein Prompt generiert. Klicken Sie „Agent synchronisieren", um den Prompt zu erstellen.
          </p>
        )}
      </CardContent>
    </Card>
