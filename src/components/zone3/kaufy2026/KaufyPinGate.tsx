...
const CORRECT_PIN = '4409';

interface KaufyPinGateProps {
  onVerified: () => void;
}

export function KaufyPinGate({ onVerified }: KaufyPinGateProps) {
  const [pin, setPin] = useState('');
  const [error, setError] = useState(false);

  const handleSubmit = () => {
    if (pin === CORRECT_PIN) {
      sessionStorage.setItem('kaufy_pin_verified', 'true');
      onVerified();
    } else {
      setError(true);
      setPin('');
  ...
  return (
    <div className="min-h-screen bg-[hsl(220,20%,8%)] flex items-center justify-center">
      <div className="flex flex-col items-center gap-6 p-8">
        <span className="text-3xl font-bold text-white tracking-tight">KAUFY</span>
