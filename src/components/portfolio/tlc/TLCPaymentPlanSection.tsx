...
import { Banknote, ChevronDown, Plus, CheckCircle } from 'lucide-react';
...
export function TLCPaymentPlanSection({ leaseId, unitId }: Props) {
  ...
  const handleCreate = () => {
    const total = parseFloat(totalArrears);
    ...
    createPlan.mutate({
      ...
      onSuccess: () => {
        setCreating(false);
        setTotalArrears(''); setMonthlyInstallment(''); setInstallmentsCount(''); setStartDate('');
      },
    });
  };
              ...
              <div key={plan.id} className="flex items-center justify-between p-2 rounded-lg border bg-card text-xs">
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="font-medium">
                      {plan.total_arrears_eur?.toLocaleString('de-DE')} € in {plan.installments_count} Raten
                    </span>
                ...
                {plan.status === 'active' && (
                  <Button
                    ...
                    onClick={() => updatePlanStatus.mutate({ planId: plan.id, installmentsPaid: (plan.installments_paid || 0) + 1 })}
                    title="Rate als gezahlt markieren"
                  >
              ...
              <div className="p-3 rounded-lg border border-dashed border-primary/30 space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    <Label className="text-[11px]">Rückstand (€) *</Label>
                    <Input className="h-7 text-xs" type="number" value={totalArrears} onChange={e => setTotalArrears(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-[11px]">Monatsrate (€) *</Label>
                    <Input className="h-7 text-xs" type="number" value={monthlyInstallment} onChange={e => setMonthlyInstallment(e.target.value)} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    <Label className="text-[11px]">Anzahl Raten *</Label>
                    <Input className="h-7 text-xs" type="number" value={installmentsCount} onChange={e => setInstallmentsCount(e.target.value)} />
                  </div>
                  <div className="space-y-1">
