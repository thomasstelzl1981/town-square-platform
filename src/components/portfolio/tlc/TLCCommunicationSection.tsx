...
export function TLCCommunicationSection({ leaseId, unitId, propertyId, tenantEmail, tenantName }: Props) {
  ...
  const handleSelectTemplate = (type: TenancyMessageType) => {
    ...
    if (template) {
      const vars: Record<string, string> = { tenant_name: tenantName || 'Mieter/in', month: format(new Date(), 'MMMM yyyy', { locale: de }), amount: '–', due_date: '–', address: '–' };
  ...
  const handleSend = () => {
    ...
    logCommunication.mutate({ leaseId, unitId, propertyId, messageType: (selectedType as TenancyMessageType) || 'general', subject, recipientEmail: tenantEmail, channel: 'email' });
    setSubject(''); setBody(''); setSelectedType('');
  };
      ...
      {selectedType && (
        <div className="space-y-2">
          ...
          <Button size="sm" className="h-7 text-xs" onClick={handleSend} disabled={logCommunication.isPending}>
            <Send className="mr-1 h-3 w-3" />Protokollieren & Senden
          </Button>
        </div>
      )}
