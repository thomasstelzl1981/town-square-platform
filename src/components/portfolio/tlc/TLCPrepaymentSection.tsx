...
import { ChevronDown, Calculator, ArrowUpDown, FileText, Copy } from 'lucide-react';
...
export function TLCPrepaymentSection({ propertyId, leaseId, currentNkAdvance = 0, tenantName, unitId }: Props) {
  ...
  const handleCalculate = () => {
    ...
    const calc = calculatePrepaymentAdjustment(
      currentNkAdvance,
      costs,
      parseInt(months) || 12,
      parseFloat(buffer) || 10
    );
    setResult(calc);

    const adj: PrepaymentAdjustment = {
      leaseId,
      unitId: unitId || '',
      tenantName: tenantName || 'Mieter/in',
      currentPrepayment: currentNkAdvance,
      actualCostsMonthly: calc.monthlyActual,
      suggestedPrepayment: calc.suggested,
      adjustmentEur: calc.adjustment,
      adjustmentPercent: calc.adjustmentPercent,
      effectiveDate: new Date(Date.now() + 90 * 86400000).toISOString().split('T')[0],
      reason: 'Anpassung auf Basis der letzten Betriebskostenabrechnung.',
    };
      ...
      <CollapsibleContent className="space-y-3 px-1 pt-2">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="space-y-1">
            <Label className="text-[10px] text-muted-foreground">Gesamt-NK (Abrechnungszeitraum) €</Label>
            <Input type="number" step="0.01" value={totalActualCosts} onChange={e => setTotalActualCosts(e.target.value)} className="h-7 text-xs" />
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] text-muted-foreground">Monate</Label>
            <Input type="number" value={months} onChange={e => setMonths(e.target.value)} className="h-7 text-xs" />
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] text-muted-foreground">Puffer %</Label>
            <Input type="number" value={buffer} onChange={e => setBuffer(e.target.value)} className="h-7 text-xs" />
          </div>
        </div>
          ...
          <div className="space-y-2 pt-2 border-t">
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-muted/40 rounded p-2">
                <p className="text-muted-foreground">Ø monatl. NK</p>
                <p className="font-semibold">{result.monthlyActual.toFixed(2)} €</p>
              </div>
              <div className="bg-muted/40 rounded p-2">
                <p className="text-muted-foreground">Vorschlag</p>
                <p className="font-semibold">{result.suggested.toFixed(2)} €</p>
              </div>
            </div>
        ...
        {letterText && (
          <div className="space-y-2 pt-2 border-t">
            ...
            <pre className="text-[10px] bg-muted/30 rounded p-2 whitespace-pre-wrap max-h-40 overflow-y-auto font-sans">
              {letterText}
            </pre>
          </div>
        )}
