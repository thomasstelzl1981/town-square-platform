...
import { Users, ChevronDown, Plus, ArrowRight } from 'lucide-react';
...
export function TLCApplicantSection({ unitId }: Props) {
  ...
  const getNextStatus = (current: ApplicantStatus): ApplicantStatus | null => {
    const flow: Record<string, ApplicantStatus> = {
      new: 'screening',
      screening: 'shortlisted',
      shortlisted: 'viewing_scheduled',
      viewing_scheduled: 'viewing_done',
      viewing_done: 'approved',
    };
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {next && (
                      <Button
                        ...
                        onClick={() => updateApplicantStatus.mutate({ id: a.id, status: next })}
                      >
                        <ArrowRight className="h-3 w-3 mr-0.5" />
                        {APPLICANT_STATUS_LABELS[next]}
                      </Button>
                    )}
                    {a.status !== 'rejected' && a.status !== 'withdrawn' && a.status !== 'approved' && (
                      <Button
                        ...
                        onClick={() => updateApplicantStatus.mutate({ id: a.id, status: 'rejected' })}
                      >
                        ✕
                      </Button>
                    )}
              ...
              <div className="p-3 rounded-lg border border-dashed border-primary/30 space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input className="h-7 text-xs" value={lastName} onChange={e => setLastName(e.target.value)} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    <Label className="text-[11px]">E-Mail</Label>
                    <Input className="h-7 text-xs" type="email" value={email} onChange={e => setEmail(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-[11px]">Telefon</Label>
                    <Input className="h-7 text-xs" value={phone} onChange={e => setPhone(e.target.value)} />
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" className="h-7 text-xs" onClick={handleCreate} disabled={createApplicant.isPending}>
                    Anlegen
                  </Button>
