...
import { useHandoverProtocol } from '@/hooks/useHandoverProtocol';
...
export function TLCHandoverSection({ leaseId, unitId, tenantId, tenantName }: Props) {
  ...
  useEffect(() => {
    if (protocols.length === 0 && !loading) fetchProtocols();
  }, [protocols.length, loading, fetchProtocols]);

  const handleCreate = async () => {
    const { error } = await createProtocol({ tenantId, leaseId, unitId, type: formType, inspectorName: inspectorName || undefined, tenantName });
    if (error) { toast.error(`Fehler: ${error.message}`); } else { setCreating(false); setInspectorName(''); }
  };
