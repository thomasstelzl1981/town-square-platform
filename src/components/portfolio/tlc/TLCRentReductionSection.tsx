...
import { TrendingDown, Plus, XCircle } from 'lucide-react';
...
export function TLCRentReductionSection({ leaseId, unitId }: Props) {
  ...
  const handleCreate = () => {
    ...
    createReduction.mutate({ leaseId, unitId, reason: reason.trim(), reductionPercent: parseFloat(percent), effectiveFrom, legalBasis: '§536 BGB', notes: notes.trim() || undefined }, {
      onSuccess: () => { setCreating(false); setReason(''); setPercent(''); setEffectiveFrom(''); setNotes(''); },
    });
  };
        ...
        <>
          {reductions.map((r: any) => (
            <div key={r.id} className="flex items-center justify-between p-2 rounded-lg border bg-card text-xs">
              ...
              {r.status === 'active' && (
                <Button variant="ghost" size="sm" className="h-6 px-1.5 text-[10px] shrink-0" onClick={() => resolveReduction.mutate({ id: r.id, effectiveUntil: new Date().toISOString().split('T')[0] })}>
          ...
          {creating ? (
            <div className="p-3 rounded-lg border border-dashed border-primary/30 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1"><Label className="text-[11px]">Minderung (%) *</Label><Input className="h-7 text-xs" type="number" min="1" max="100" value={percent} onChange={e => setPercent(e.target.value)} /></div>
                <div className="space-y-1"><Label className="text-[11px]">Wirksam ab *</Label><Input className="h-7 text-xs" type="date" value={effectiveFrom} onChange={e => setEffectiveFrom(e.target.value)} /></div>
              </div>
              {guidelines && (
              ...
              <div className="space-y-1"><Label className="text-[11px]">Anmerkungen</Label><Textarea className="text-xs min-h-[40px]" value={notes} onChange={e => setNotes(e.target.value)} /></div>
