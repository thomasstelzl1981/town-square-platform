...
import { FileSignature, Copy, FileText } from 'lucide-react';
...
export function TLCContractSection({ leaseData, contactId }: Props) {
  ...
  const sections = useMemo(() => {
    ...
    try { return generateSections(leaseData as LeaseContractData); } catch { return null; }
  }, [leaseData, generateSections]);

  const handleCopyToClipboard = () => {
    if (!leaseData?.landlordName) return;
    try { const text = generateFullText(leaseData as LeaseContractData); navigator.clipboard.writeText(text); toast.success('Vertrag in Zwischenablage kopiert'); } catch { toast.error('Vertragsdaten unvollständig'); }
  };
  ...
  const handleSendToBrief = () => {
    ...
    try {
      ...
      const params = new URLSearchParams({ subject: 'Mietvertrag', prompt: text, ...(contactId ? { contactId } : {}) });
      navigate(`/portal/office/brief?${params.toString()}`);
    } catch { toast.error('Vertragsdaten unvollständig'); }
  };
  ...
  return (
    <div className="space-y-2">
      <h4 className={DESIGN.TYPOGRAPHY.LABEL}>
        <FileSignature className="h-3.5 w-3.5 inline mr-1.5" />
        Vertragsgenerator
      </h4>
      {!hasData ? (
            ...
            <div className="space-y-3 p-2">
              {sections?.map((s, i) => (
                <div key={i} className="text-xs">
                  ...
                  <p className="text-muted-foreground whitespace-pre-line mt-0.5 text-[11px] leading-relaxed">{s.content}</p>
                </div>
              ))}
            </div>
          </ScrollArea>
