...
const SHOP_SECTIONS: { key: string; label: string; desc: string; isAffiliate?: boolean; intro?: ReactNode }[] = [
  {
    ...
    intro: (
      <div className="rounded-xl p-5 space-y-2" style={{ backgroundColor: C.sandLight }}>
        <p className="text-sm leading-relaxed" style={{ color: C.bark }}>
          Wir empfehlen <a href="https://www.lakefields.de" target="_blank" rel="noopener noreferrer" className="font-semibold underline underline-offset-2" style={{ color: C.forest }}>Lakefields</a> — eine Manufaktur am Bodensee, die Premium-Hundefutter in Lebensmittelqualität herstellt. Entwickelt mit Tierärzten, ohne Getreide, ohne künstliche Zusatzstoffe und ohne Zucker.
        </p>
      </div>
    ),
  },
  ...
  },
  {
    ...
    intro: (
      <div className="rounded-xl p-5" style={{ backgroundColor: C.sandLight }}>
        <p className="text-sm leading-relaxed" style={{ color: C.bark }}>
          Unsere Auswahl an hochwertigem Hundespielzeug und Zubehör von Fressnapf — vom Klassiker KONG bis zum Intelligenzspielzeug.
        </p>
      </div>
    ),
  },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold" style={{ color: C.bark }}>{label}</h3>
          <p className="text-xs" style={{ color: C.barkMuted }}>{desc}</p>
        </div>
        {isAffiliate && (
          <Badge variant="outline" className="text-[10px]" style={{ borderColor: C.forest, color: C.forest }}>
            Partner
          </Badge>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-8 text-center rounded-lg border border-dashed" style={{ borderColor: C.sand }}>
              ...
              <Card className="h-full border hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer" style={{ borderColor: C.sand, background: 'white' }}>
                <CardContent className="p-3 flex flex-col h-full">
                  ...
                  <div className="flex flex-col flex-1 items-center text-center gap-1 pt-2">
                    {p.badge && (
                      <Badge className="text-[10px] text-white border-0" style={{ backgroundColor: C.forest }}>
                        {p.badge}
                      </Badge>
                    )}
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="max-w-4xl mx-auto px-5 space-y-10">
        ...
        {/* ═══ LENNOX TRACKER TEASER ═══ */}
        <section>
          <Card className="overflow-hidden border" style={{ borderColor: C.sand, background: 'white' }}>
            <CardContent className="p-6 flex flex-col sm:flex-row items-center gap-4">
              <div className="p-3 rounded-xl" style={{ backgroundColor: `${C.forest}12` }}>
                <Radar className="h-8 w-8" style={{ color: C.forest }} />
              </div>
              <div className="flex-1 text-center sm:text-left">
                <h3 className="font-semibold" style={{ color: C.bark }}>Lennox GPS Tracker</h3>
                <p className="text-sm mt-1" style={{ color: C.barkMuted }}>
                  Echtzeit-Ortung, Aktivitätstracking und Geofencing für deinen Liebling.
                </p>
              </div>
              <Button variant="outline" size="sm" className="gap-1.5" disabled style={{ borderColor: C.forest, color: C.forest }}>
