/**
 * OttoLeadPool — Lead-Pool tab for Otto² Advisory Desk
 * Shows all leads from otto_advisory_kontakt + otto_advisory_finanzierung
 */
...
export default function OttoLeadPool() {
  ...
  async function fetchLeads() {
    ...
    const [leadsRes, orgsRes] = await Promise.all([
      supabase.from('leads').select('*').in('source', ['otto_advisory_kontakt', 'otto_advisory_finanzierung']).order('created_at', { ascending: false }),
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Lead-Pool — Otto² Advisory</CardTitle>
        </CardHeader>
        <CardContent>
          {leads.length === 0 ? (
                  ...
                  <TableRow key={lead.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline" className="text-xs">
                        {lead.source === 'otto_advisory_finanzierung' ? 'Finanzierung' : 'Beratung'}
                      </Badge>
                    </TableCell>
          ...
          {detailLead && (
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-muted-foreground">Typ:</span> {detailLead.source === 'otto_advisory_finanzierung' ? 'Finanzierung' : 'Beratung'}</div>
