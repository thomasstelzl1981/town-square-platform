...
const iconMap: Record<string, any> = {
  ...
  'file-check': Icons.FileCheck,
  'package': Icons.Package,
  'loader': Icons.Loader,
  'video': Icons.Video,
  'palette': Icons.Palette,
  'layout-template': Icons.LayoutTemplate,
  'user-plus': Icons.UserPlus,
};

function getIcon(iconKey: string) {
  return iconMap[iconKey] || Icons.LayoutGrid;
}
            ...
            return (
              <Card key={tile.tile_code} className="overflow-hidden">
                ...
                <Link
                  ...
                  className="block p-5 bg-gradient-to-br from-primary/10 to-primary/5 hover:from-primary/20 hover:to-primary/10 transition-colors"
                >
                ...
                {/* Sub-tiles Grid */}
                {tile.sub_tiles.length > 0 && (
                  <div className="grid grid-cols-2 divide-x divide-y divide-border">
                    {tile.sub_tiles.slice(0, 4).map((sub, idx) => {
                      const SubIcon = getIcon(sub.icon_key);
                      return (
                        <Link
                          key={idx}
                          to={sub.route}
                          className="p-3 flex flex-col items-center gap-1.5 hover:bg-muted/50 transition-colors text-center"
                        >
                          <SubIcon className="h-4 w-4 text-muted-foreground" />
                          <span className="text-[11px] font-medium leading-tight">{sub.title}</span>
                        </Link>
                      );
                    })}
                  </div>
