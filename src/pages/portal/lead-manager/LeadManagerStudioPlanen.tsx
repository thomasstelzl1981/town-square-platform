/**
 * Lead Manager — Kampagne planen (MOD-10)
 * Migrated from SelfieAdsPlanen with brand_context support
 */
...
import { Megaphone, Image, Sparkles, User, FileText, Check, Upload, Loader2 } from 'lucide-react';
...
type PlanFormData = {
  ...
  personalization: { name: string; region: string; claim: string; phone: string; email: string };
  creatives: Record<string, SlotCreative>;
};

const DEFAULT_CAPTIONS: Record<string, string> = {
  T1: 'Sichere Rendite — Bis zu 6% p.a. mit Kapitalanlage-Immobilien',
...
export default function LeadManagerStudioPlanen() {
  const navigate = useNavigate();
  const [isGenerating, setIsGenerating] = useState(false);

  const [form, setForm] = useState<PlanFormData>({
    brandContext: 'kaufy',
    goal: 'Lead-Generierung',
  ...
  const toggleSlot = (key: string) => {
    setForm(prev => ({
      ...prev,
      selectedSlots: prev.selectedSlots.includes(key) ? prev.selectedSlots.filter(s => s !== key) : [...prev.selectedSlots, key],
    }));
  };
  ...
  const updatePersonalization = (field: string, value: string) => {
    setForm(prev => ({ ...prev, personalization: { ...prev.personalization, [field]: value } }));
  };

  const updateCreative = (slotKey: string, field: 'caption' | 'cta', value: string) => {
    setForm(prev => ({
      ...prev,
      creatives: { ...prev.creatives, [slotKey]: { ...prev.creatives[slotKey], [field]: value } },
    }));
  };
  ...
  const handleNavigateToSummary = () => {
    sessionStorage.setItem('leadManagerPlanData', JSON.stringify(form));
    navigate('/portal/lead-manager/studio/summary');
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 md:px-6 space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Lead Manager — Social-Media-Mandat konfigurieren</p>
      </div>
      ...
      <Card className="glass-card">
        <CardContent className="p-5 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-medium">Brand / Marke</h2>
          </div>
          <div className="flex flex-wrap gap-2">
            {BRAND_OPTIONS.map(b => (
              <Badge
                key={b.key}
                variant={form.brandContext === b.key ? 'default' : 'outline'}
                className="cursor-pointer"
                onClick={() => setForm(prev => ({ ...prev, brandContext: b.key }))}
              >
                {form.brandContext === b.key && <Check className="h-3 w-3 mr-1" />}
                {b.label}
              </Badge>
            ))}
          </div>
      ...
      <Card className="glass-card">
        <CardContent className="p-5 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-medium">Kampagnen-Parameter</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input type="number" value={form.budget} onChange={e => setForm(prev => ({ ...prev, budget: Number(e.target.value) }))} min={500} step={100} />
            </div>
            <div className="space-y-2">
              <Label>Regionen (max. 3)</Label>
              <Input placeholder="z.B. München, Berlin, Hamburg" value={form.regions} onChange={e => setForm(prev => ({ ...prev, regions: e.target.value }))} />
            </div>
          </div>
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {PRESETS.map(p => (
                <Badge key={p} variant={form.presets.includes(p) ? 'default' : 'outline'} className="cursor-pointer" onClick={() => togglePreset(p)}>
                  {form.presets.includes(p) && <Check className="h-3 w-3 mr-1" />}{p}
                </Badge>
              ))}
            </div>
          </div>
      ...
      <Card className="glass-card">
        <CardContent className="p-5 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-medium">Template-Slots (5 CI Templates)</h2>
          </div>
            ...
            {TEMPLATE_DEFS.map(t => {
              ...
              return (
                <div key={t.key} onClick={() => toggleSlot(t.key)}
                  className={`relative rounded-xl border-2 p-4 cursor-pointer transition-all ${isSelected ? 'border-primary bg-primary/5 shadow-sm' : 'border-dashed border-border bg-muted/20 hover:border-primary/40'}`}>
                  <Badge variant={isGenerated ? 'default' : isSelected ? 'secondary' : 'outline'} className="absolute top-2 right-2 text-xs">
                  ...
                  <div className="h-16 w-full rounded-lg bg-muted/50 flex items-center justify-center mb-3">
                    {isGenerated ? <Sparkles className="h-6 w-6 text-primary/60" /> : <Image className="h-6 w-6 text-muted-foreground/50" />}
                  </div>
          ...
          <p className="text-xs text-muted-foreground">{form.selectedSlots.length}/5 Slots gewählt</p>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card">
        <CardContent className="p-5 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-medium">Personalisierung</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label>Name</Label>
              <Input placeholder="Ihr Name" value={form.personalization.name} onChange={e => updatePersonalization('name', e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>Region</Label>
              <Input placeholder="z.B. München & Umgebung" value={form.personalization.region} onChange={e => updatePersonalization('region', e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>Claim (max. 80 Zeichen)</Label>
              <Input placeholder="Ihr persönlicher Claim" maxLength={80} value={form.personalization.claim} onChange={e => updatePersonalization('claim', e.target.value)} />
        ...
        <CardContent className="p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-medium">Creatives generieren</h2>
            </div>
            <Button onClick={handleGenerate} disabled={form.selectedSlots.length === 0 || isGenerating} className="gap-2">
              {isGenerating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
              {isGenerating ? 'Generiere...' : `Generieren (${form.selectedSlots.length} Slots)`}
            </Button>
          </div>
          {form.selectedSlots.length === 0 && !hasCreatives && (
            <p className="text-sm text-muted-foreground py-4 text-center">Wählen Sie oben mindestens einen Template-Slot aus</p>
          )}
          {hasCreatives && (
                  ...
                  <div key={slotKey} className="p-4 rounded-lg bg-muted/30 border border-border/50 space-y-3">
                    ...
                    <div className="flex gap-2">
                      {creative.slides.map((sl, idx) => (
                        <div key={idx} className="h-20 flex-1 rounded-md bg-gradient-to-br from-muted/80 to-muted/40 border border-border/50 flex flex-col items-center justify-center p-1">
                          ...
                          <span className="text-xs text-muted-foreground/70 text-center mt-0.5 leading-tight">{sl}</span>
                        </div>
                      ))}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <Input value={creative.caption} onChange={e => updateCreative(slotKey, 'caption', e.target.value)} className="text-xs" />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">CTA</Label>
                        <Input value={creative.cta} onChange={e => updateCreative(slotKey, 'cta', e.target.value)} className="text-xs" />
                      </div>
                    </div>
                  </div>
                );
      ...
      {hasCreatives && form.selectedSlots.length > 0 && (
        <Card className="glass-card border-primary/30">
          <CardContent className="p-5 space-y-4">
            <div className="flex items-center gap-2 mb-2">
              <FileText className="h-4 w-4 text-primary" />
              <h2 className="font-medium">Mandat-Zusammenfassung</h2>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div>
                <span className="text-muted-foreground text-xs">Brand</span>
                <p className="font-medium">{BRAND_OPTIONS.find(b => b.key === form.brandContext)?.label}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{form.selectedSlots.length} Slots</p>
              </div>
            </div>
            <Button className="w-full gap-2" onClick={handleNavigateToSummary}>
              Zur Zusammenfassung
            </Button>
          </CardContent>
        </Card>
