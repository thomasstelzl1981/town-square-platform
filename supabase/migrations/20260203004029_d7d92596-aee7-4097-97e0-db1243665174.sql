-- Fix seed function: add monthly_rent which is NOT NULL
...
BEGIN
  ...
  VALUES 
    ('00000000-0000-4000-a000-000000000110', tenant_id, 'Familie Mustermann', 'PRIVATE', 'VERMÖGENSVERWALTUNG', 98000, 42.00, true, false, NULL, 1, true, 'Zusammenveranlagung Ehepaar - Steuerklasse III/V')
  ON CONFLICT (id) DO UPDATE SET
    name = EXCLUDED.name,
  ...
  -- ============ 6. LEASE (with monthly_rent as required NOT NULL) ============
  INSERT INTO leases (id, tenant_id, unit_id, tenant_contact_id, start_date, end_date, monthly_rent, rent_cold_eur, nk_advance_eur, deposit_amount_eur, payment_due_day, status)
  VALUES 
    ('00000000-0000-4000-a000-000000000120', tenant_id, '00000000-0000-4000-a000-000000000002', '00000000-0000-4000-a000-000000000103', '2022-06-01', NULL, 837, 682, 155, 2046, 3, 'active')
  ON CONFLICT (id) DO UPDATE SET
    monthly_rent = EXCLUDED.monthly_rent,
    rent_cold_eur = EXCLUDED.rent_cold_eur,
    nk_advance_eur = EXCLUDED.nk_advance_eur,
    updated_at = now();
  ...
  VALUES 
    ('00000000-0000-4000-a000-000000000201', tenant_id, 'SOT-D-000201', 'Exposé_Ankauf.pdf', 'a0000000-0000-4000-a000-000000000001/demo/01/Expose.pdf', 'application/pdf', 150000, 'expose_buy', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000202', tenant_id, 'SOT-D-000202', 'Grundbuchauszug.pdf', 'a0000000-0000-4000-a000-000000000001/demo/03/Grundbuch.pdf', 'application/pdf', 200000, 'land_register', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000203', tenant_id, 'SOT-D-000203', 'Teilungserklaerung.pdf', 'a0000000-0000-4000-a000-000000000001/demo/04/Teilung.pdf', 'application/pdf', 350000, 'division_declaration', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000204', tenant_id, 'SOT-D-000204', 'Grundriss.pdf', 'a0000000-0000-4000-a000-000000000001/demo/05/Grundriss.pdf', 'application/pdf', 80000, 'floorplan', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000205', tenant_id, 'SOT-D-000205', 'Kaufvertrag.pdf', 'a0000000-0000-4000-a000-000000000001/demo/07/Kaufvertrag.pdf', 'application/pdf', 450000, 'purchase_contract', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000206', tenant_id, 'SOT-D-000206', 'Mietvertrag.pdf', 'a0000000-0000-4000-a000-000000000001/demo/08/Mietvertrag.pdf', 'application/pdf', 180000, 'lease_contract', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000207', tenant_id, 'SOT-D-000207', 'WEG_Wirtschaftsplan.pdf', 'a0000000-0000-4000-a000-000000000001/demo/10/WEG.pdf', 'application/pdf', 120000, 'weg_plan', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000208', tenant_id, 'SOT-D-000208', 'Versicherung.pdf', 'a0000000-0000-4000-a000-000000000001/demo/13/Versicherung.pdf', 'application/pdf', 95000, 'insurance', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000209', tenant_id, 'SOT-D-000209', 'Darlehensvertrag.pdf', 'a0000000-0000-4000-a000-000000000001/demo/15/Darlehen.pdf', 'application/pdf', 280000, 'loan_contract', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000210', tenant_id, 'SOT-D-000210', 'Energieausweis.pdf', 'a0000000-0000-4000-a000-000000000001/demo/12/Energie.pdf', 'application/pdf', 110000, 'energy_cert', 'property', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000211', tenant_id, 'SOT-D-000211', 'ESt_Bescheid.pdf', 'a0000000-0000-4000-a000-000000000001/demo/SD/ESt.pdf', 'application/pdf', 160000, 'tax_assessment', 'applicant', 'golden_path_seed'),
    ('00000000-0000-4000-a000-000000000212', tenant_id, 'SOT-D-000212', 'Lohnsteuer.pdf', 'a0000000-0000-4000-a000-000000000001/demo/SD/Lohnst.pdf', 'application/pdf', 75000, 'income_proof', 'applicant', 'golden_path_seed')
  ON CONFLICT (id) DO UPDATE SET
    name = EXCLUDED.name,
