...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    v_record_id := OLD.id;
    -- For profiles: use active_tenant_id; for others: use tenant_id
    v_tenant_id := CASE 
      WHEN v_table_name = 'profiles' THEN OLD.active_tenant_id
      ELSE OLD.tenant_id 
    END;
    
    -- Skip audit if no tenant_id available (orphan cleanup scenario)
  ...
  -- For profiles: use active_tenant_id; for others: use tenant_id
  v_tenant_id := CASE 
    WHEN v_table_name = 'profiles' THEN NEW.active_tenant_id
    ELSE NEW.tenant_id 
  END;
  ...
  -- Skip audit if no tenant_id available
  IF v_tenant_id IS NULL THEN
    RETURN NEW;
  END IF;
