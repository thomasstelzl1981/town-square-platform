...
// Pure-JS MD5 (RFC 1321) — Deno has no native MD5
...
function parseDigestChallenge(header: string): Record<string, string> {
  ...
  // qop may appear without quotes
  const qopMatch = header.match(/qop=([^,\s]+)/);
  if (qopMatch && !params.qop) {
    params.qop = qopMatch[1].replace(/"/g, '');
  }
  return params;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      cameraResponse = await fetch(snapshotUrl, { signal: controller.signal });
    } catch (fetchErr) {
      clearTimeout(timeout);
      const message = fetchErr instanceof Error ? fetchErr.message : "Connection failed";
      return new Response(
        JSON.stringify({ error: "Camera unreachable", details: message, status: "offline" }),
        { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    // Step 2: Handle 401 — Digest or Basic Auth
    if (cameraResponse.status === 401 && camera.auth_user && camera.auth_pass) {
      ...
      if (wwwAuth.toLowerCase().startsWith("digest")) {
        ...
        camAuthHeader = buildDigestAuth("GET", uri, camera.auth_user, camera.auth_pass, challenge);
      } else {
        // Basic Auth fallback
        camAuthHeader = `Basic ${btoa(`${camera.auth_user}:${camera.auth_pass}`)}`;
      }
      ...
      try {
        ...
        const message = fetchErr instanceof Error ? fetchErr.message : "Connection failed";
        return new Response(
          JSON.stringify({ error: "Camera unreachable after auth", details: message, status: "offline" }),
          { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    } else if (cameraResponse.status === 401) {
      clearTimeout(timeout);
      return new Response(
        JSON.stringify({ error: "Kamera verlangt Zugangsdaten — bitte in den Kamera-Einstellungen hinterlegen", status: "auth_error" }),
        { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (!cameraResponse.ok) {
      return new Response(
        JSON.stringify({ error: "Camera returned error", status_code: cameraResponse.status, status: "error" }),
        { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
